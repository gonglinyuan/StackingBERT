...
class Trainer(object):
    ...
    def load_doubled_checkpoint(self, filename, reset_optimizer=False, optimizer_overrides=None):
        extra_state, _, last_optim_state = utils.load_bert_state(filename, self.get_model(), upgrade=False)
        if last_optim_state is not None and not reset_optimizer:
            self._build_optimizer()
            self.optimizer.load_state_dict(last_optim_state, optimizer_overrides)
        return extra_state
